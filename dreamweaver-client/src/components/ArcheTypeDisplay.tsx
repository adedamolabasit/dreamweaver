...
import { Brain, User, Castle, Compass, Crown, Sword, Heart, Wand2 } from 'lucide-react';
...
// Map archetype names to their corresponding icons
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {data.secondaryArchetypes.map((archetype, index) => (
            <div key={index} className="p-3 rounded-lg bg-purple-900/20 border border-purple-500/10 flex items-center">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {data.symbolMeanings.map((symbol, index) => (
            <div key={index} className="p-4 rounded-lg bg-blue-900/20 border border-blue-500/10">
              <h4 className="text-blue-300 font-medium text-sm mb-1">{symbol.symbol}</h4>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {data.emotionalTone.map((emotion, index) => (
              <span key={index} className="px-3 py-1 rounded-full bg-pink-500/20 text-pink-200 text-xs">
                {emotion}
              </span>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {data.potentialConflicts.map((conflict, index) => (
              <span key={index} className="px-3 py-1 rounded-full bg-amber-500/20 text-amber-200 text-xs">
                {conflict}
              </span>
            ))}
          </div>
