...
const InterpretationDisplay: React.FC<InterpretationDisplayProps> = ({ data }) => {
  return (
    <div className="w-full">
      <div className="mb-6">
        ...
        <div className="p-5 rounded-xl bg-gradient-to-br from-blue-900/30 to-purple-900/20 backdrop-blur-sm border border-blue-500/20 mb-6">
          <p className="text-blue-100/90 leading-relaxed">{data.interpretation}</p>
        </div>
      </div>
          ...
          <div className="p-4 rounded-lg bg-pink-900/20 border border-pink-500/20">
            <div className="flex flex-wrap gap-2 mb-4">
              {data.emotions.map((emotion, index) => (
                <span key={index} className="px-3 py-1 rounded-full bg-pink-500/30 text-pink-200 text-xs">
                  {emotion}
                </span>
              ))}
            </div>
            <p className="text-pink-100/70 text-sm">
              These emotions form the foundation of your dream's emotional landscape, 
              influencing the tone and direction of potential narratives.
            </p>
          </div>
        </div>
          ...
          <div className="p-4 rounded-lg bg-blue-900/20 border border-blue-500/20">
            <div className="flex flex-wrap gap-2">
              {data.symbols.map((symbol, index) => (
                <span key={index} className="px-3 py-1 rounded-full bg-blue-500/30 text-blue-200 text-xs">
                  {symbol}
                </span>
              ))}
            </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          {data.potentialStories.map((story, index) => (
            <div key={index} className="p-4 rounded-lg bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-500/10">
